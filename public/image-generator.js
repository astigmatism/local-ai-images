...
function renderControlChrome() {
  ...
  if (slideshowButton) {
    const showSlideshowControl = Boolean(state.autoGenerateEnabled);
...
function createPendingJob(payload, options = {}) {
  ...
  const submissionSource = options.source === 'auto' ? 'auto' : 'manual';
  ...
  return {
    ...
    metadata: {
      clientStatus: 'Submitting...',
      ...(submissionSource === 'auto' ? { clientSubmissionSource: 'auto' } : {})
    },
    timings: {},
    error: null
  };
...
function latestCompletedAutoGeneratedJob() {
  return currentJobs().find((job) => isAutoGeneratedJob(job) && isCompletedImageJob(job)) || null;
}
...
async function prewarmSelectedModel() {
  ...
  try {
    ...
    if (state.autoGenerateEnabled) stopAutoGenerate(`Auto-Generate stopped because checkpoint loading failed. ${message}`, false);
...
async function finalizeGenerationResult(clientId, submittedPayload, result) {
  ...
  const autoJob = isAutoGeneratedClientId(clientId);
  ...
  if (job.status === 'canceled') {
    ...
    setStatus(`Generation canceled for job ${job.id || 'n/a'}.${autoJob ? ' Auto-Generate turned off.' : ''}`);
...
async function submitGenerationJob(clientId, payload, options = {}) {
  const autoJob = options.source === 'auto' || isAutoGeneratedClientId(clientId);
  try {
    ...
    if (job?.status === 'canceled' || error?.body?.error?.code === 'IMAGE_JOB_CANCELED') {
      ...
      setStatus(`Generation canceled: ${message}${autoJob ? '. Auto-Generate turned off.' : ''}`);
      return;
    }
    ...
    setStatus(`${prefix}: ${message}${autoJob ? '. Auto-Generate turned off.' : ''}`, false);
  } finally {
    forgetAutoGeneratedClientId(clientId);
    renderControlChrome();
  }
...
function submitGenerationRequest(source = 'manual') {
  if (source !== 'auto' && state.autoGenerateEnabled) {
    setAutoGenerateEnabled(false, { message: 'Auto-Generate turned off for manual generation.', ok: true });
  }
  ...
  const pendingJob = createPendingJob(payload, { source });
  if (source === 'auto') markJobAsAutoGenerated(pendingJob);
  addPendingJob(pendingJob);
  const prefix = source === 'auto' ? 'Auto-Generate queued' : 'Queued generation request';
