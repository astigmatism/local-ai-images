...
Important image-service settings:
...
Replace `<user>` with the logged-in Ubuntu username. Do not set a default LLM model for normal image-generation deployment.
...
The tests use Node's built-in test runner and do not require real GPUs or ComfyUI. Optional legacy compatibility tests use mocks and do not require an Ollama service.
...
With default image-only settings, these checks must not produce Ollama connection warnings.
